-- Migration: Fix Package Credit Consumption in Generate Slots
-- Decrementa o saldo do pacote ao criar o agendamento automaticamente.
      ...
      WHILE v_slots_per_item > 0 AND v_safety_count < 1000 LOOP
        IF EXTRACT(DOW FROM v_slot_date)::INTEGER = ANY(v_cp.preferred_weekdays) THEN
          ...
          IF NOT EXISTS (
            ...
            -- 2. Atualizar consumo do crédito
            IF v_credit_id IS NOT NULL THEN
              UPDATE public.package_credits
              SET used_quantity = used_quantity + 1,
                  remaining_quantity = GREATEST(0, remaining_quantity - 1),
                  updated_at = now()
              WHERE id = v_credit_id;
            END IF;

            -- 3. Inserir no slot do pacote vinculado ao appointment
